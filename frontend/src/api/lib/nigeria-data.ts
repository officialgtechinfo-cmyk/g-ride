...
export const getStates = (): string[] => NIGERIA_STATES.map((s) => s.name);
...
// Fare calculation helper (simplified)
export const calculateFare = (mode: "G-Cab" | "G-Courier"): number => {
  const baseFare = mode === "G-Cab" ? 2500 : 1500;
  const variation = Math.floor(Math.random() * 1500) + 500;
  return baseFare + variation;
};